...
- If the app won't get far enough to reach Settings, launch it with the `WEBKIT_DISABLE_DMABUF_RENDERER=1` environment variable set, or pass `--safe-rendering` on the command line (this rewrites the saved setting immediately, before anything else runs).
