...
The dedicated TV build from the Play Store stays receiver-only; it never gets this browse UI. To get it on a TV, sideload the mobile APK instead - the same "Sideloading on Fire TV" instructions below work on any Android TV device, not just Fire TV.
